...
RAFT_NODES_COUNT=${RAFT_NODES_COUNT} ORDERER_GENERAL_LISTENPORT=${RAFT0_PORT:-${ORDERER_GENERAL_LISTENPORT:-7050}} ORDERER_NAME=${ORDERER_NAME} \
    CONSENTER_ID=1 ./$BASEDIR/0_raft-start-1-node.sh
returnCode=$?
/bin/sleep 8
echo "Start Raft node 1"
ORDERER_OPERATIONS_LISTENADDRESS=0.0.0.0:9091 ORDERER_GENERAL_LISTENPORT=${RAFT1_PORT:-7150} ORDERER_NAME=${ORDERER_NAME_1} \
    CONSENTER_ID=2 ./$BASEDIR/0_raft-start-1-node.sh --no-deps orderer
returnCode=$(($returnCode + $?))
echo "Start Raft node 2"
ORDERER_OPERATIONS_LISTENADDRESS=0.0.0.0:9092 ORDERER_GENERAL_LISTENPORT=${RAFT2_PORT:-7250} ORDERER_NAME=${ORDERER_NAME_2} \
