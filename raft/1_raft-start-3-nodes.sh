...
ORDERER_GENESIS_PROFILE=RaftOrdererGenesis COMPOSE_PROJECT_NAME=${ORDERER_NAME_2}.${DOMAIN} ORDERER_GENERAL_LISTENPORT=${RAFT1_PORT:-7150} ORDERER_NAME=${ORDERER_NAME_2} docker-compose ${DOCKER_COMPOSE_ORDERER_ARGS} up -d orderer
sleep 5
echo "Start first Raft node 2"
ORDERER_GENESIS_PROFILE=RaftOrdererGenesis COMPOSE_PROJECT_NAME=${ORDERER_NAME_3}.${DOMAIN} ORDERER_GENERAL_LISTENPORT=${RAFT2_PORT:-7250} ORDERER_NAME=${ORDERER_NAME_3} docker-compose ${DOCKER_COMPOSE_ORDERER_ARGS} up -d orderer
sleep 5
