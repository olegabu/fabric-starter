...
export DOMAIN ORDERER_DOMAIN WWW_PORT RAFT_NODES_COUNT RAFT_NODES_NUMBERING_START

ORDERER_NAME=${ORDERER_NAME_PREFIX}${RAFT_NODES_NUMBERING_START}
...
COMPOSE_PROJECT_NAME=${ORDERER_NAME}.${ORDERER_DOMAIN} ORDERER_NAME=${ORDERER_NAME} ORDERER_NAME_PREFIX=${ORDERER_NAME_PREFIX} ORDERER_GENESIS_PROFILE=Raft3OrdererGenesis ORDERER_GENERAL_LISTENPORT=${RAFT1_PORT:-7150} docker-compose ${DOCKER_COMPOSE_ORDERER_ARGS} up -d cli.orderer

RAFT_NUMBER=$(($RAFT_NUMBER+1))
ORDERER_NAME=${ORDERER_NAME_PREFIX}$RAFT_NUMBER
COMPOSE_PROJECT_NAME=${ORDERER_NAME}.${ORDERER_DOMAIN} ORDERER_NAME=${ORDERER_NAME} ORDERER_NAME_PREFIX=${ORDERER_NAME_PREFIX} ORDERER_GENESIS_PROFILE=Raft3OrdererGenesis ORDERER_GENERAL_LISTENPORT=${RAFT2_PORT:-7250} docker-compose ${DOCKER_COMPOSE_ORDERER_ARGS} up -d cli.orderer
