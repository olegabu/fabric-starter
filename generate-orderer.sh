...
EXECUTE_BY_ORDERER=1 runCLI "cryptogen generate --config=crypto-config/cryptogen-orderer.yaml && chown $UID -R crypto-config/"
...
EXECUTE_BY_ORDERER=1 runCLI "configtxgen -configPath crypto-config/ -outputBlock crypto-config/configtx/genesis.pb -profile OrdererGenesis -channelID orderer-system-channel \
