...
#source "${BASEDIR}"/../libslib/util/util.sh
#source "${BASEDIR}"/../libslib.sh
...
(cd ${FABRIC_DIR} && pwd && PEER0_PORT=${PEER0_PORT} ORG=${ORG} ./chaincode-instantiate.sh ${TEST_CHANNEL_NAME} ${CHAINCODE_NAME} | tee -a ${FSTEST_LOG_FILE} > "${output}")
