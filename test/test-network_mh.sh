...
if [ "${MULTIHOST}" = true ]; then
    eval $(docker-machine env org1.example.com)
fi
...
echo runTest PEER0_PORT=${LOCAL_PEER_PORT} TEST_CHANNEL_NAME=${TEST_CHANNEL_NAME} ORG=org2 ${BASEDIR}/${testinterface}/join-channel.sh

runTest PEER0_PORT=${LOCAL_PEER_PORT} TEST_CHANNEL_NAME=${TEST_CHANNEL_NAME} ORG=org2 ${BASEDIR}/${testinterface}/join-channel.sh
...
runTest PEER0_PORT=${LOCAL_PEER_PORT} TEST_CHANNEL_NAME=${TEST_CHANNEL_NAME} ORG=org2 ${BASEDIR}/verify/test-join-channel.sh
    eval $(docker-machine env org1.example.com)
...
runTest PEER0_PORT=${LOCAL_PEER_PORT} CHAINCODE_NAME=${CHAINCODE_NAME} TEST_CHANNEL_NAME=${TEST_CHANNEL_NAME} ORG=org2 ${BASEDIR}/verify/test-chaincode-install.sh
