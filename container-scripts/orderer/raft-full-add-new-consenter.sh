...
certificationsToEnv orderer ${NEWCONSENTER_DOMAIN} ${GENERATE_DIR}
...
mergeListIntoChannelConfig ${CHANNEL} "${GENERATE_DIR}/configtx/updated_config_msp.json" 'channel_group.groups.Orderer.values.ConsensusType.value.metadata.consenters' \
...
mergeListIntoChannelConfig ${CHANNEL} "${GENERATE_DIR}/configtx/updated_config_concenter.json" 'channel_group.values.OrdererAddresses.value.addresses' \
