...
function installChaincode {
    ...
    CORE_PEER_ADDRESS=$PEEER_ORG_NAME.$DOMAIN:$PEER0_PORT peer chaincode install -n $chaincodeName -v $chaincodeVersion -p $chaincodePath -l $lang
}
...
function installChaincodePackage() {
    ...
    CORE_PEER_ADDRESS=$PEEER_ORG_NAME.$DOMAIN:$PEER0_PORT peer chaincode install $chaincodeName
}
...
function initChaincode() {
    ...
    CORE_PEER_ADDRESS=$PEEER_ORG_NAME.$DOMAIN:$PEER0_PORT peer chaincode ${operation} -n $chaincodeName -v ${chaincodeVersion} -c "${arguments}" -o ${ORDERER_ADDRESS} -C $channelName ${ORDERER_TLSCA_CERT_OPTS} $privateCollectionParam $endorsementPolicyParam
}
