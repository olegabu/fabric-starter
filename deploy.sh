...
if [[ -z "$AGENT_MODE" ]]; then ldap_yaml="-f docker-compose-ldap.yaml"; fi
docker_compose_args=${DOCKER_COMPOSE_ARGS:-"-f docker-compose.yaml -f docker-compose-couchdb.yaml $ldap_yaml"}
