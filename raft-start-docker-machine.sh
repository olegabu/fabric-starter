...
function main() {
  ...
  docker pull ${DOCKER_LOCAL_REGISTRY:-${DOCKER_REGISTRY:-docker.io}}/${FABRIC_STARTER_REPOSITORY:-olegabu}/fabric-tools-extended:${FABRIC_STARTER_VERSION:-latest}
  ...
  docker run --rm  -v ./:/currdir ${DOCKER_LOCAL_REGISTRY:-${DOCKER_REGISTRY:-docker.io}}/${FABRIC_STARTER_REPOSITORY:-olegabu}/fabric-tools-extended:${FABRIC_STARTER_VERSION:-latest} chown -R `id -u`  crypto-config
