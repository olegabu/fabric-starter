...
function main() {
  ...
  docker pull ${DOCKER_LOCAL_REGISTRY:-${DOCKER_REGISTRY:-docker.io}}/olegabu/fabric-tools-extended:${FABRIC_STARTER_VERSION:-snapshot-0.13-2.3}
  ...
  docker run --rm  -v ./:/currdir ${DOCKER_LOCAL_REGISTRY:-${DOCKER_REGISTRY:-docker.io}}/olegabu/fabric-tools-extended:${FABRIC_STARTER_VERSION:-snapshot-0.13-2.3} chown -R `id -u`  crypto-config
...
function prepareOrg() {
  ...
  bash -c "\
    ...
    export FABRIC_STARTER_HOME=${WORK_DIR}; \
    connectMachine ${org};  \
    docker pull ${DOCKER_REGISTRY:-docker.io}/olegabu/fabric-tools-extended:${FABRIC_STARTER_VERSION:-snapshot-0.13-2.3}; \
    docker pull ${DOCKER_REGISTRY:-docker.io}/olegabu/fabric-starter-rest:${FABRIC_STARTER_VERSION:-snapshot-0.13-2.3}; \
